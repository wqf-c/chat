...
public:
	static long long currentTimeMillis() {
		...
		auto duration_in_ms =
			duration_cast<milliseconds>(time_now.time_since_epoch());
	...
	static std::vector<size_t> range(size_t begin, size_t until,
		size_t step = 1) {
		using std::vector;
		vector<size_t> rst{};
	...
	static auto uniformDoubleDistribution(double min, double max,
		int seed = INT_MIN) {
		static std::random_device rd;
		static std::mt19937 gen{ seed == INT_MIN ? rd() : seed };
